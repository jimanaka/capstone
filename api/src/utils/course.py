...
def register_course(username: str, course_id: str, db_ctx: MongoContext) -> Tuple[Dict, int]:
    ...
    if "registered_courses" in user_doc:
        if course_id in user_doc["registered_courses"]:
            result = corsify_response(jsonify(msg="Already registered for course")), HTTP.OK.value
            return result
    ...
    if result.modified_count > 0:
        result = corsify_response(jsonify(msg="Successfully registered course")), HTTP.OK.value
    else:
        result = corsify_response(jsonify(msg="Failed to register course")), HTTP.SERVICE_UNAVAILABLE.value
    return result
